...
export default function RecentActivity() {
  ...
  return (
    <section className="rounded-xl border border-border bg-card mobile-card-padding space-y-4 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 min-[390px]:gap-4 divide-y md:divide-y-0 md:divide-x divide-border">
        {/* Left Column: Continue Reading */}
        <div className="pb-4 md:pb-0 md:pr-3 min-[390px]:md:pr-4 space-y-2.5 min-[390px]:space-y-3">
          ...
          {continueItems.length > 0 ? (
              ...
              {continueItems.map(item => (
                <div key={item.href} className="relative group">
                  <Link
                    ...
                    className="block rounded-lg border border-primary/20 bg-primary/5 mobile-card-padding hover:border-primary/45 hover:bg-primary/10 active:bg-primary/15 transition-all duration-200"
                  >
            ...
            <div className="rounded-lg border border-dashed border-border p-3 min-[360px]:p-4 min-[390px]:p-6 text-center select-none">
        ...
        <div className="pt-4 md:pt-0 md:pl-3 min-[390px]:md:pl-4 space-y-2.5 min-[390px]:space-y-3">
          <div className="flex items-center justify-between">
            <div>
            ...
            <div className="grid gap-2 grid-cols-1 sm:grid-cols-2">
              {recentItems.slice(0, 6).map((item, idx) => (
                <Link
                  key={`${item.href}-${idx}`}
                  href={item.href}
                  className="rounded-lg border border-border bg-card mobile-card-padding hover:border-foreground/20 hover:bg-muted/30 active:bg-muted/50 transition-all duration-200 flex flex-col justify-between"
                >
