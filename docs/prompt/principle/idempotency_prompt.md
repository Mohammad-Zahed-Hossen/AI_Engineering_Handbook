...
# AENS Principle Resource Prompt — Idempotency
...
* **Target Principle Name:** `Idempotency`
* **Principle Category:** `systems`
* **Problem Statement:** AI engineering pipelines become unreliable and unsafe when operations are not idempotent, causing duplicate data ingestion, inconsistent model states, corrupted checkpoints, and unpredictable side effects when jobs are retried, replayed, or executed concurrently in distributed environments.
...
Never explain:
* what idempotency means in general
* basic software engineering concepts
* beginner API concepts
* what "idempotency" is at a basic level
...
> **"What is the Idempotency principle, why does it matter for AI engineering systems, and how can I identify and apply it correctly?"**
...
Provide a 2-4 sentence overview describing the Idempotency principle in AI engineering systems, focusing on operational safety, retry reliability, and state consistency in distributed training and inference pipelines.

---

## Problem

State the engineering problem: When AI engineering operations—such as data writes, model checkpoint saves, feature store updates, or inference result logging—are not designed to be idempotent, retries after failures, duplicate message delivery, or concurrent execution produce inconsistent state, data corruption, or resource leaks, making systems unpredictable and difficult to debug in production.
