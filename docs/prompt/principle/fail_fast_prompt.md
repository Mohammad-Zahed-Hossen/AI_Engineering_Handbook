...
# AENS Principle Resource Prompt — Fail Fast
...
* **Target Principle Name:** `Fail Fast`
* **Principle Category:** `systems`
* **Problem Statement:** AI engineering systems waste enormous engineering time and compute resources when configuration errors, data pipeline failures, or model training pathologies are detected late in long-running workflows, leading to cascading failures, silent data corruption, and delayed feedback loops that make root cause analysis prohibitively expensive.
...
Never explain:
* what "fail fast" means in general
* basic software engineering concepts
* beginner API concepts
* what "fail fast" is at a basic level
...
> **"What is the Fail Fast principle, why does it matter for AI engineering systems, and how can I identify and apply it correctly?"**
...
Provide a 2-4 sentence overview describing the Fail Fast principle in AI engineering systems, focusing on rapid error detection, minimized blast radius, and accelerated feedback loops in training and inference pipelines.

---

## Problem

State the engineering problem: When AI engineering pipelines defer validation, sanity checks, and invariant assertions to the end of long-running jobs, errors propagate undetected through hours or days of computation, corrupting downstream artifacts, exhausting budgets, and compressing the time available for debugging before production deadlines.
...
* **Tags:** architecture, systems, reliability, debugging, pipeline-design, error-handling
* **Aliases:** fail-immediately, rapid-failure, early-termination, defensive-programming, assert-early
* **Keywords:** fail-fast, error-detection, validation, assertions, preconditions, invariants, sanity-checks, early-termination, blast-radius, feedback-loop, defensive-design
* **Search Tokens:** fail fast, fail immediately, rapid failure, early termination, defensive programming, assert early, validation pipeline, sanity check, precondition check, invariant assertion, error propagation, blast radius minimization
* **Difficulty:** Intermediate
* **Domain:** systems
* **Engineering Area:** reliability_engineering
* **Estimated Reading Time:** 15-20 minutes
* **Prerequisites:** None
* **Recommended Next:** idempotency, separation-of-concerns, observability, circuit-breaker, graceful-degradation
* **Cross-Links:**
...
* `domain` - "systems"
* `difficulty` - "intermediate"
* `engineering_area` - "reliability_engineering"
