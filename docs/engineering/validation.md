...
- Examples of valid IDs: `numpy`, `scikit-learn`, `random-forest`, `rag`
- Examples of invalid IDs: `numpy.json`, `NumPy`, `scikit_learn`, `bge-large-en-v1.5` (periods not allowed)
...
- **Type Compatibility**: The referenced type must match the target's actual type.
- **Legacy String Detection**: Detects old string-based alternatives (e.g., `"cupy"`) and requires ContentRef format (`{ "id": "cupy", "type": "package" }`).
